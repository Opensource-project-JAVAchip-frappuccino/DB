...
public class ConnectServer {
    ...
    public void CheckList(int id){
        try {
            ...
            PreparedStatement pstmt = this.conn.prepareStatement(sql);
            //pstmt.setInt(4, id); //입력한 id를 통해서 서버와 확인
            //변수 삽입
            pstmt.setString(1, "소프트웨어학과"); //major
            //int i=0;
            pstmt.setInt(2, 2); //grade (0:professor)
            pstmt.setInt(3, 0); //gender (0:male 1:female)
            pstmt.setInt(4, id); //id (number)
            pstmt.setInt(5, 1); //leader
            pstmt.setInt(6, 1); //announce
            pstmt.setInt(7, 3); //ppt
            pstmt.setInt(8, 3); //frontend
            pstmt.setInt(9, 4); //backend
            pstmt.setInt(10, 3); //selfability
            // setInt 부분 반복문으로 줄일 것.
            pstmt.setBoolean(11, false); //teammate (0:false, 1:true)
